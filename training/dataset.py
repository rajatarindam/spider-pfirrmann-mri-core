...
from configs.paths import DATASET_CSV
...
class PfirrmannDataset(Dataset):
    """
    Dataset for single-disc Pfirrmann grading.

    Each row in dataset.csv represents one disc sample.
    ...
    def __init__(
        ...
        if split is not None:
            ...
            self.df = self.df[
                self.df["Split"].astype(str).str.lower()
                == split
            ].reset_index(
                drop=True
            )
        ...
        if invalid_grades:
            ...
            raise ValueError(
                f"Invalid Pfirrmann grades found: "
                f"{invalid_grades}"
            )
